...
"""}]
        ...
        if response.status_code == 200:
            data = response.json()
            respuesta = data.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", None)
            return respuesta
        else:
            return None
...
st.markdown('<div class="footer"><p>SG-SST PHVA - IA con Gemini © 2024</p></div>', unsafe_allow_html=True)
