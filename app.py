...
<style>
    ...
    .main-header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 2rem; border-radius: 20px; color: white; text-align: center; margin-bottom: 2rem; animation: slideIn 0.5s; }
    .fase-card { background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); padding: 1rem; border-radius: 15px; margin: 0.5rem; text-align: center; transition: all 0.3s; }
    ...
    .fase-actual { border: 2px solid #ffcc00; background: rgba(255,204,0,0.15); transform: scale(1.02); }
    .metric-card { background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); padding: 1.5rem; border-radius: 15px; text-align: center; color: white; transition: transform 0.3s; }
    .metric-card:hover { transform: translateY(-5px); }
    .step-card { background: rgba(255,255,255,0.1); padding: 1rem; border-radius: 10px; margin: 0.5rem 0; border-left: 4px solid #667eea; }
    [data-testid="stSidebar"] { background: linear-gradient(180deg, #0f2027, #203a43); }
    [data-testid="stSidebar"] * { color: white; }
    .stButton > button { background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none; border-radius: 10px; font-weight: bold; }
    .footer { text-align: center; padding: 1rem; margin-top: 2rem; color: rgba(255,255,255,0.5); }
...
def login_screen():
    ...
    with col2:
        st.markdown('<div style="background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); padding: 2rem; border-radius: 20px; text-align: center;"><img src="https://cdn-icons-png.flaticon.com/512/2917/2917995.png" width="100"><h1>🔄 SG-SST PHVA</h1><h3>Ciclo PHVA - 6 Fases Completas</h3>', unsafe_allow_html=True)
...
def dashboard():
    ...
    fases_recomendaciones = [
        "🎯 Registra la información de tu empresa",
        "⚠️ Identifica peligros por proceso",
        "📊 Evalúa probabilidad y severidad",
        "📋 Define acciones correctivas",
        "🚀 Programa capacitaciones",
        "📈 Monitorea indicadores"
    ]
    st.info(f"**Próxima acción:** {fases_recomendaciones[st.session_state.fase_actual-1]}")
...
def fase1_diagnostico():
    ...
    with st.form("fase1_form"):
        ...
        with col1:
            ...
            sector = st.selectbox("🏭 Sector", ["Construcción", "Manufactura", "Servicios", "Minería", "Salud"], index=0)
        with col2:
            trabajadores = st.number_input("👥 Trabajadores", min_value=1, value=st.session_state.empresa.get("trabajadores", 45))
            ciudad = st.text_input("📍 Ciudad", value=st.session_state.empresa.get("ciudad", ""))
            arl = st.selectbox("🛡️ ARL", ["Positiva", "Sura", "Colpatria", "Bolívar"], index=0)
        if st.form_submit_button("💾 Guardar", use_container_width=True):
            st.session_state.empresa = {"nombre": nombre, "nit": nit, "sector": sector, "trabajadores": trabajadores, "ciudad": ciudad, "arl": arl}
    ...
    if st.session_state.empresa.get("nombre"):
        ...
        col1, col2 = st.columns(2)
        with col1: st.write(f"**Empresa:** {st.session_state.empresa.get('nombre')}")
        with col2: st.write(f"**Trabajadores:** {st.session_state.empresa.get('trabajadores')}")
    ...
    with tab2:
        with st.form("nuevo_peligro"):
            ...
            with col2:
                ...
                niveles_texto = {"I":"🔴 Riesgo Extremo", "II":"🟠 Riesgo Alto", "III":"🟡 Riesgo Medio", "IV":"🟢 Riesgo Bajo"}
                st.info(f"**Nivel calculado:** {niveles_texto[nivel]}")
...
def fase3_riesgos():
    ...
    if st.session_state.peligros:
        for p in st.session_state.peligros:
            ...
            st.markdown(f'<div class="step-card {riesgo_clase}"><h4>📍 {p["peligro"]}</h4><p>Proceso: {p["proceso"]} | Prob: {p["probabilidad"]} | Sev: {p["severidad"]}</p><p><strong>Nivel: {p["nivel"]}</strong></p><p>Controles: {p["controles"]}</p></div>', unsafe_allow_html=True)
...
def fase6_seguimiento():
    ...
    with col1:
        ...
        st.metric("Índice de Frecuencia", "2.5")
        st.metric("Días perdidos", "12")
...
def asistente_ia():
    ...
    for msg in st.session_state.ia_messages:
        if msg["role"] == "user":
            st.markdown(f'<div style="background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 15px; padding: 1rem; margin: 0.5rem 0;">👤 {msg["content"]}</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div style="background: rgba(102,126,234,0.2); border-radius: 15px; padding: 1rem; margin: 0.5rem 0;">🤖 {msg["content"]}</div>', unsafe_allow_html=True)
    if prompt := st.chat_input("Consulta..."):
        st.session_state.ia_messages.append({"role": "user", "content": prompt})
        respuestas = {
            "fase": f"Estamos en Fase {st.session_state.fase_actual}. Progreso: {calcular_progreso()}%",
            "peligro": "Usa la metodología GTC-45: evalúa probabilidad (1-4) y severidad (1-3)",
            "trabajador": "Registra trabajadores en el módulo correspondiente",
            "capacitacion": "Programa capacitaciones anuales obligatorias según normativa"
        }
        respuesta = next((v for k, v in respuestas.items() if k in prompt.lower()), "¿Necesitas ayuda con alguna fase específica?")
...
if not st.session_state.authenticated:
    ...
    with st.sidebar:
        ...
        st.markdown(f'<div style="background: #667eea; padding: 0.5rem; border-radius: 10px; text-align: center;"><strong>📍 FASE ACTUAL</strong><br>{fases_nombres[st.session_state.fase_actual]}</div>', unsafe_allow_html=True)
        st.progress(calcular_progreso() / 100)
